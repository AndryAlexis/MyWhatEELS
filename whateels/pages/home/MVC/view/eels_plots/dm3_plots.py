...
class DM3Plots:
    ...
    def create_layout(self):
        ...
        # Setup range slider for energy loss axis
        eloss_coords = self.model.dataset.coords[self.model.constants.ELOSS]
        
        self.range_slider = pn.widgets.RangeSlider(
            name='Range',
            start=float(eloss_coords[0]),
            end=float(eloss_coords[-1]),
            value=(float(eloss_coords[0]), float(eloss_coords[-1]))
        )
        self.range_slider.param.watch(self._on_range_change, 'value')
    ...
    def _on_range_change(self, event):
        # This is a placeholder for range slider callback logic
        # You should implement spectrum update logic here if needed
        pass
